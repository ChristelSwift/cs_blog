...
Working with copulas means that we can separate the problems of modelling the margins and modelling the dependence structure. These two steps can be done independently.
...
# check stations with the lowest correlations:
spearman_cor_db %>% 
  arrange(desc(cor)) %>% 
  head()
```
...
rbind(
  ...
  labs(title = "actual vs simulated station distributions") +
  theme(legend.position = "bottom")
```
...
compare_corrs %>% 
  ...
  labs(
    title = "V1: station pairwise Spearman correlations",
    x = "actual correlations", y = "simulated correlations"
    )
```
...
Then we calculate the percentage reach for all pairs of stations and all categories: P(X = cat0 and Y = cat0) etc...

```{r warning = FALSE}
# create all possible pairs of stations:
my_grid <- expand.grid(
  var1 = stations,
  var2 = stations,
  stringsAsFactors = FALSE
...
When looking at the pairwise distributions we notice that:\
- the non-users of both stations (cat0) are well aligned with small differences.\
- the light-users of both stations (cat1) are systematically under-estimated.
...
for(i in 2:nrow(station_ranking)){
  ...
  my_list[[i-1]] <- data.frame(
    station_combination = paste(my_stations$name, collapse = ", "),
    n1 = ncol(actual_mat),
    n2 = ncol(sim_mat),
    actual_pc = sum(actual_comb_sum > 0) / length(actual_comb_sum),
    sim_pc = sum(sim_comb_sum > 0) / length(sim_comb_sum)
...
Calculate the tetrachoric correlation for all pairs of stations that include the zero value:
...
for(i in 1:nrow(all_pairs)){
  ...
  # transform the 1st station to binary:
  x1 <- db %>% 
    select(!!sym(v1)) %>% 
    rename(v1 = !!sym(v1)) %>% 
  ...
  # transform the 2nd station to binary:
  x2 <- db %>% 
    select(!!sym(v2)) %>% 
    rename(v2 = !!sym(v2)) %>% 
...
tcc_df <- rbind(
  ...
  # diagonal of 1s:
  diag,
  # lower triangle obtained by inversing station1 and station2:
  all_pairs %>% 
    select(-phi_coef) %>% 
    rename(
      var2 = var1,
...
rbind(
  ...
  geom_density() +
  facet_wrap(~name, ncol = 3) +
  labs(title = "V2: actual vs simulated station distributions") +
  theme(legend.position = "bottom")
```
...
compare_corrs %>% 
  ...
  expand_limits(x = c(-0.2, 0.3), y = c(-0.2, 0.3)) +
  labs(
    title = "V2: station pairwise Spearman correlations",
    x = "actual correlations", y = "simulated correlations"
    )
```
